...
// 共享工具定义: embedded 和 standalone 版本都使用这些定义
// standalone 版本通过 withProjectRoot() 注入 projectRoot 参数
...
For bulk removal, use batch_remove_bookmarks.`,
  ...
  {
    name: 'remove_group',
    description: 'Remove a bookmark group and all its bookmarks. This is a destructive operation.',
    inputSchema: {
      type: 'object',
      properties: {
        groupId: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        confirm: {
          type: 'boolean',
          description: 'Must be set to true to confirm the operation. This prevents accidental deletion.'
        }
      },
      required: ['confirm']
    }
...
// projectRoot 参数定义, standalone 版本给所有工具注入这个参数
...
// 给所有工具注入 projectRoot 参数 (standalone 版本使用)
export function withProjectRoot(tools: Tool[]): Tool[] {
...
// standalone 版本独有的工具
export const STANDALONE_TOOLS: Tool[] = [
  {
    name: 'set_workspace',
