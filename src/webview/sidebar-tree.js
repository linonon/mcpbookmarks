...
function renderBookmarkHeaderTree(bookmark, hasChildren, isCollapsed, depth = 0) {
  ...
  const getBookmarkDisplayOrder = window.getBookmarkDisplayOrder || ((b) => {
    if (!b) return undefined;
    if (Number.isFinite(b.displayOrder)) return b.displayOrder;
    if (Number.isFinite(b.order)) return b.order;
    return undefined;
  });
